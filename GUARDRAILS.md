...
1. Roda: `.claude/guardrails/bin/guardrails check` (lint + typecheck + testes) passa.
2. Nenhum segredo novo: `.claude/guardrails/bin/guardrails scan` limpo.
...
- Precisa de uma sessão sem hooks → `.claude/guardrails/bin/guardrails off` (e `... on` depois).
